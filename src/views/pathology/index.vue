<template>
  <div class="home-page" ref="pathologyPage">
    <!-- 搜索区 -->
      ...
      <div class="search-block-wrap">
        ...
        <a-row class="top-tips">
          <a-alert type="warning" showIcon>
            <div slot="message">支持查询证件号包括身份证号、护照、军官证、港澳通行证、台湾居住证。(注:主要提供病理检查报告、病理诊断数据调阅)</div>
          </a-alert>
        </a-row>
      </div>
    </div>
      ...
      <div class="form-query">
        <div class="filter-row">
          <div class="filter-item">
            <span class="filter-label">送检日期：</span>
            <a-range-picker
          ...
          <div class="filter-item">
            <span class="filter-label">送检科室：</span>
            <a-select
...
import { getPathologyList } from '@/api/pathology'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          dataIndex: 'age',
          width: '70px',
          customRender: (text) => (text || text === 0 ? `${text}岁` : '-'),
        },
        {
          title: '门诊/住院号',
          key: 'visitNo',
        ...
        },
        {
          ...
          key: 'specimen',
          dataIndex: 'specimen',
          // 多点取材后标本名是一长串，190px 会折成三行把行高撑起来
          width: '260px',
          customRender: (text) => text || '-',
        },
        {
          ...
          dataIndex: 'sendDate',
          width: '110px',
          sorter: (a, b) => new Date(a.sendDate).valueOf() - new Date(b.sendDate).valueOf(),
        },
        {
          title: '病理诊断',
          key: 'diag',
        ...
        },
        {
          ...
          customRender: (text, record) => {
            // 列表里只会有已出报告的记录，不再有「报告未出、点不进去」这种情况
            return [
              this.$createElement('a', { on: { click: () => this.viewDetail(record) } }, '查看病理报告')
            ]
          },
        },
      ],
  ...
  computed: {
    ...
    // 送检机构就是医院名称
    organOptions() {
      return [...new Set(this.dataSource.map((item) => item.sendOrgan).filter(Boolean))]
    },
    deptOptions() {
    ...
    filteredData() {
      ...
      return this.dataSource.filter((item) => {
        if (start && item.sendDate < start) return false
        if (end && item.sendDate > end) return false
        if (organName && item.sendOrgan !== organName) return false
  ...
  methods: {
    ...
    async queryList() {
      ...
      const { data = [] } = await getPathologyList({
    ...
    viewDetail(record) {
      const obj = {
        id: record.pathNo,
        text: `${record.name} ${record.specimen}`,
        status: '0',
        type: 'report',
        typeName: '病理报告',
        new_label: '0',
        key: `/detail/pathology/report?id=${record.pathNo}&type=report&patient_name=${this.postData.patient_name}&patient_id_card=${this.postData.patient_id_card}`,
      }
      // 已存在同名页签就不再新增，避免重复打开
