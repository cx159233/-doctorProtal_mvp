<template>
  <div class="home-page" ref="outpatientPage">
    <!-- 搜索区 -->
      ...
      <div class="search-block-wrap">
        ...
        <a-row class="top-tips">
          <a-alert type="warning" showIcon>
            <div slot="message">支持查询证件号包括身份证号、护照、军官证、港澳通行证、台湾居住证。(注:主要提供门诊就诊记录、门诊电子病历数据调阅)</div>
          </a-alert>
        </a-row>
      </div>
    </div>
      ...
      <div class="form-query">
        <div class="filter-row">
          <div class="filter-item">
            <span class="filter-label">就诊日期：</span>
            <a-range-picker
          ...
          <div class="filter-item">
            <span class="filter-label">就诊科室：</span>
            <a-select
...
import { getOutpatientList } from '@/api/outpatient'
...
export default {
  ...
  methods: {
    ...
    async queryList() {
      ...
      const { data = [] } = await getOutpatientList({
    ...
    viewDetail(record) {
      const obj = {
        ...
        typeName: '门诊病历',
        new_label: '0',
        key: `/detail/outpatient/report?id=${record.visitId}&organCode=${record.organCode}&new_label=0&type=report&patient_name=${this.postData.patient_name}&patient_id_card=${this.postData.patient_id_card}`,
      }
      // 已存在同名页签就不再新增，避免重复打开
