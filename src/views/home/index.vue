<template>
  <div class="home-page" ref="homePage">
    <!-- 搜索区 -->
    <div class="search-block" v-if="!isHideSelect">
    ...
    <!-- 查询条件 -->
    <div class="wrap" :style="{paddingTop: isHideSelect ? 0 : '12px'}">
      <a-row v-if="tableTip && activeSearchMode === '1'">
        <a-col :span="24">
          <!-- 提示语 -->
      ...
      <div class="search-mode-tabs" v-if="!isHideSelect">
        <a-tabs
          v-model="activeSearchMode"
          type="card"
          @change="switchSearchMode"
        >
          <a-tab-pane key="1">
            <template slot="tab">
              <span class="tab-label">省内查询</span>
              <span v-if="intraProvinceCount > 0" class="tab-badge">{{ intraProvinceCount > 99 ? '99+' : intraProvinceCount }}</span>
            </template>
          </a-tab-pane>
          <a-tab-pane key="2">
            <template slot="tab">
              <span class="tab-label">省外调阅</span>
              <span v-if="crossProvinceCount > 0" class="tab-badge">{{ crossProvinceCount > 99 ? '99+' : crossProvinceCount }}</span>
            </template>
          </a-tab-pane>
        </a-tabs>
      </div>
      <!-- 省内/省外共用同一套查询条件 -->
      <div v-show="!isHideSelect">
          ...
          <a-row class="row1 flex1">
            ...
            <a-col :sm="smCol" :md="mdCol" :xl="xsCol">
              <a-form-item label="医院名称：" class="hospital-name">
                ...
                :getPopupContainer="(triggerNode) => triggerNode.parentNode"
                  ...
                  @search="(value) => fetch(value,'hospitalList','hospital')"
                  :not-found-content="fetching ? undefined : null"
                  @focus="(value) => fetch(value,'hospitalList','hospital')"
                >
                  <a-spin v-if="fetching" slot="notFoundContent" size="small" />
                  <a-select-option v-for="item in hospitalList" :label="item.hospital_name" :key="item.hospital_code">
                      {{ item.hospital_name }}
                    </a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :sm="smCol" :md="mdCol" :xl="xsCol">
              <a-form-item label="检查设备类型：">
                ...
                :getPopupContainer="(triggerNode) => triggerNode.parentNode"
                  ...
                  :placeholder="`请选择`"
                  :filter-option="filterOptionHospitalName"
                  @search="(value) => fetch(value,'checkTypeList','modality')"
                  :not-found-content="fetching ? undefined : null"
                  @focus="(value) => fetch(value,'checkTypeList','modality')"
                >
                  <a-spin v-if="fetching" slot="notFoundContent" size="small" />
                  <a-select-option v-for="item in checkTypeList" :label="item" :key="item">
                    {{ item }}
                  </a-select-option>
                </a-select>
              </a-form-item>
            ...
            <a-col :sm="smCol" :md="mdCol" :xl="xsCol" class="ckpt_name">
              ...
              :getPopupContainer="(triggerNode) => triggerNode.parentNode"
                ...
                :placeholder="`请选择`"
                :filter-option="false"
                @search="(value) => fetch(value,'ckptArr','ckpt_name')"
                :not-found-content="fetching ? undefined : null"
                @focus="(value) => fetch(value,'ckptArr','ckpt_name')"
              >
                <a-spin v-if="fetching" slot="notFoundContent" size="small" />
                <a-select-option
              ...
              <!-- 查看时间范围 -->
            <a-col :sm="12" :md="mdCol" :xl="xsCol" class="isOpen time-range" v-if="time_view_range !== ''">
              <a-form-item label="查看时间范围：">
                <!-- 时间范围目前只有「全部」，做成下拉框和其它条件的形式一致 -->
                <a-select
                  :getPopupContainer="(triggerNode) => triggerNode.parentNode"
                  v-model="time_view_range"
                >
    ...
    <div class="query-table">
      <Table
        ...
        :totalNum="tableDataSource.length"
  ...
  <!-- 跨省调阅详情页（/home/cross-province/* 路由）：列表页不渲染，避免加载不可达地址 -->
  <div v-show="crossProvinceIframeVisible" class="cross-province-content">
    <div class="iframe_container" style="width: 100%">
      <iframe
        ref="myIframe"
        class="cmdb_iframe"
        :src="iframeSrc"
        ...
        style="width:100%"
      ></iframe>
      <div v-show="!iframeSrc" class="none">
        <a-empty description="未检测到跨省调阅地址" />
      </div>
    </div>
  </div>
    <!-- 协议内容 -->
...
import { getSms, queryTableList, queryCrossProvinceList, queryHospitalName, queryCheckType, getTimeRange, createAuth, getAuthStatus, confirmAuth, getAppQRCode, getMiniQRCode,smsValidate,getHistoryPhone,getAuthMethod } from '@/api/image-list'
...
export default {
  ...
  data() {
    return {
      ...
      activeSearchMode: '1', // 搜索模式，默认省内查询
      iframeUrl: '',
      smCol: 8,
      xsCol: 8,
      mdCol: 8,
      ...
      columns: [
        ...
        },
        {
          ...
          sorter: (a, b) => new Date(a.check_date).valueOf() - new Date(b.check_date).valueOf(),
        },
        {
          title: '医院名称',
          key: 'hospital_name',
        ...
        },
        {
          ...
          dataIndex: 'action',
          customRender: (text, record, index) => {
            const child1 = this.$createElement(
              'a',
              {
                attrs: {
                  class: record.has_dicom === '0' ? 'aDisable' : '',
                },
                style: {
                  marginRight: '23px',
                },
                on: {
                  click: () => {
                    return record.has_dicom === '0' ? null : this.viewDetail(record, 'dicom')
                  },
                },
              },
              '查看影像'
            ...
            const child2 = this.$createElement(
              ...
              {
                attrs: {
                  class: record.has_report === '0' ? 'aDisable' : '',
                },
                on: {
                  click: () => {
                    return record.has_report === '0' ? null : this.viewDetail(record, 'report')
                  },
                },
              },
              '查看报告'
      ...
      time: 0,
      timer: null,
      ckptTimer:null, // 检查部位定时器
      fetching: false,//下拉框搜索
      ckptArr:[],//检查部位
      ckpt_name:undefined,//检查部位名称
      ...
      intraProvinceCount: 0, // 省内查询总数
      crossProvinceCount: 0, // 跨省调阅总数
      crossProvinceList: [] // 省外调阅列表数据
    }
  },
  methods: {
    ...
    fetch(_value,arr,tag) {
      ...
      this.ckptTimer = setTimeout(async () => {
        ...
        if(patient_id_card && patient_name){
          // if (this.hospitalList.length) return
          this.searchLoading = true
          const formData = new FormData()
          formData.append('patient_name',patient_name)
          formData.append('patient_id_card',patient_id_card)
          const { data = {} } = await queryHospitalName(formData)
          this[arr] = data?.[tag]||[]
    ...
    async confirmAuthorize() {
      ...
      if (this.authorizeType === 1 || this.authorizeType === 5) { // 江苏健康通或者第三方授权
        ...
        if (data) {
          ...
          this.setPoint('patient_data_check','authorize_success','',{authorize_type:`${this.authorizeType}`})
   ...
   async getAuthMethodX(){
    ...
    async authStatus(params) {
      ...
      if (!data) { // 未授权，要创建授权
        ...
        if (data && data.auth_id)  {
          ...
          this.setPoint('enter_js_iis','authorize_loading_success')
        }
    ...
    async checkSms() {
      ...
      if (data === 'ok') { // 授权成功
        ...
        if (data === 'ok') { // 确认授权成功
          ...
          this.setPoint('patient_data_check','authorize_success','',{authorize_type:`2`})
          this.closeAuth()
    ...
    getURL(){
      return process.env.NODE_ENV === 'production' ? `/ksdy-org-view/` : `http://localhost:5173/ksdy-org-view/`
    },
    switchSearchMode(key) {
      this.activeSearchMode = key
      this.iframeUrl = ''
    },
    getIframeUrl(isDefault) {
      const urlFromQuery = this.$route.query.url
      if(isDefault || this.$route.path.includes('/home/index')){
        return this.getURL()
      }else{
        return urlFromQuery?decodeURIComponent(urlFromQuery):this.getURL()
      }
    },
    // 患者姓名失去焦点
    blurName(e) {
    ...
    reset() {
      this.$nextTick(() => {
        this.$refs.queryForm && this.$refs.queryForm.form.resetFields()
      })
    ...
    async trackingEvent(params) {
      if(this.$route.path.includes('/home/cross-province')){
        return
      }
    ...
    // 切换右边日期进行查询
    switchChange(val) {
      if (this.patient_name && this.patient_id_card) {
      } else {
        this.$message.error({
          content: '患者姓名和证件号码为必填字段',
          duration: 2,
        })
        return
      }
      const obj = {
        ...
        patient_name: this.patient_name, // 患者姓名
        patient_id_card: this.patient_id_card, // 证件号码
        hospital_code: this.hospital_code || '', // 医院编号
        intra: this.intra || '',
    ...
    disabledDate(current) {
      const nowDate = new Date().valueOf() // 现在日期的毫秒数
      const rangDate = 180 * 24 * 60 * 60 * 1000 // 当前日期往前推180天 算出180总毫秒数
      const startDate = nowDate - rangDate // 当前日期 - 相差总毫秒数 = 90前天的日期
      // 往前推90天
      return (current && current > new Date()) || current.valueOf() < startDate
    },
    // 日期改变
    ...
    viewDetail(record, type) {
      this.setPoint('user_query_behavior',type==='dicom'?'user_image_btn_click':'user_report_btn_click','',{study_primary_id:record.id})
      if (type === 'dicom') { // 记录影像埋点
        ...
        const trackingObj = {
          ...
          type: 'pacs_view',
            ...
            patient_name: this.postData.patient_name,
            doc_name: this.userInfo.user_name,
            hospital_name: this.$store.getters.hospitalName,
            patient_idcard: this.postData.patient_id_card,
          }
        }
      ...
      const text = record.pat_name + ' ' + record.item_name
      const obj = {
        // 组装数据
        id: record.id,
        text,
        ...
        key:
          type === 'dicom'
            ? `/detail/image?uid=${this.userInfo.uid}&id=${record.id}&type=dicom&has_dicom=${record.has_dicom}&has_report=${record.has_report}&hospital_code=${record.hospital_code}&pat_name=${record.pat_name}&item_name=${record.item_name}&patient_id_card=${this.postData.patient_id_card}&app_dpt_name=${this.urlParams.app_dpt_name||''}&chk_modality=${record.category_name}&patient_name=${this.postData.patient_name}&view_record_id=${this.urlParams.view_record_id||''}&new_label=0`
            : `/detail/report?uid=${this.userInfo.uid}&id=${record.id}&type=report&has_dicom=${record.has_dicom}&has_report=${record.has_report}&hospital_code=${record.hospital_code}&pat_name=${record.pat_name}&item_name=${record.item_name}&patient_id_card=${this.postData.patient_id_card}&app_dpt_name=${this.urlParams.app_dpt_name||''}&chk_modality=${record.category_name}&patient_name=${this.postData.patient_name}&view_record_id=${this.urlParams.view_record_id||''}&new_label=0`,
      }
      ...
      this.$router.push({
        ...
        query: {
          ...
          chk_modality:record.category_name,
          patient_name:this.postData.patient_name,
    ...
    async queryList(params) {
      ['hospital_code' ,'intra' , 'ckpt_name'].forEach(flag=>{
        const data=this[flag]
        if(data&&Array.isArray(data)){
          data.forEach(v=>{
            if(['ckpt_name'].includes(flag)) {
              params.append(flag, v)
            }else{
              params.append(`${flag}[]`, v)
            }
        })
      ...
      if (this.postData.patient_name && this.postData.patient_id_card) {
        ...
        // 省外调阅与省内共用查询条件，一次查询同时刷新两个页签的数据
        const crossParams = new FormData()
        params.forEach((value, key) => crossParams.append(key, value))
        const [intraRes, crossRes] = await Promise.all([
          queryTableList(params),
          queryCrossProvinceList(crossParams)
        ])
        const { data = [], code = '' } = intraRes
        this.dataSource = (data && Array.isArray(data) && data) || []
        this.intraProvinceCount = this.dataSource.length // 设置省内查询总数
        const crossData = crossRes?.data
        this.crossProvinceList = (crossData && Array.isArray(crossData) && crossData) || []
        this.crossProvinceCount = this.crossProvinceList.length // 设置省外调阅总数
        let visit_data_time = ''
        const obj2 = {}
    ...
    handleResize(event) {
      const { type, height, number,data,message } = event.data
      this.crossProvinceCount = number
      if (type === 'iframeResize' && height) {
        this.$refs.myIframe.style.height = height + 'px'
      }else if(type === 'viewReport'){
        const currentKey = `/home/cross-province/${data.id}/${data.source}?activeSearchMode=2&url=${encodeURIComponent(data.url)}&text=${data.text}`
        const tabPane = this.$store.getters.tabPane
        const bol = tabPane.some(v => v && v.key && v.key.includes(currentKey))
        if (!bol) {
          this.$store.dispatch('PushTabPane', {
          text:data.text,
          key: currentKey
        })
        }
        this.$router.push({
          path: `/home/cross-province/${data.id}/${data.source}`,
          query:{
            activeSearchMode:2,
    ...
    judgeCurrent(){
      ...
      if(this.$route.path.includes('/home/index')){
        this.iframeUrl = this.getIframeUrl(true)
      }
      if(this.$route.path.includes('/home/cross-province')){
      ...
      const tabPane = this.$store.getters.tabPane
        if (tabPane.length < 1) {
          ...
          key: decodeURIComponent(this.$route.fullPath)
        })
        }
    }
    window.addEventListener('message', this.handleResize)
  ...
  async created() {
    ...
    if(!this.urlParams.hospital_code&&!this.urlParams.org_id){
      this.initSearch()
    }
  },
  mounted() {
  ...
  watch: {
    $route: {
      handler(to, from) {
        // 监听路由变化，从 cross-province 进入时处理
        if (to.query.activeSearchMode === '2') {
          this.activeSearchMode = '2'
          this.iframeUrl = this.getIframeUrl()
        } else if (to.query.activeSearchMode === '1') {
          this.activeSearchMode = '1'
          this.iframeUrl = ''
        }
      },
  ...
  computed: {
    ...
    // 两个页签共用一张表，省外调阅展示省外的数据
    tableDataSource() {
      return this.activeSearchMode === '2' ? this.crossProvinceList : this.dataSource
    },
    // 跨省调阅的 iframe 只在 /home/cross-province/* 详情路由下渲染
    crossProvinceIframeVisible() {
      return this.$route.path.includes('/home/cross-province')
    },
    // 列表页不加载跨省调阅地址，避免请求不可达的地址
    iframeSrc() {
      return this.crossProvinceIframeVisible && this.iframeUrl ? this.iframeUrl : 'about:blank'
    }
  },
}
</script>
